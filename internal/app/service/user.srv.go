...
func (a *UserSrv) GetByUserName(ctx context.Context, Name string, opts ...schema.UserQueryOptions) (*schema.User, error) {
	item, err := a.UserRepo.GetByUserName(ctx, Name, opts...)
	if err != nil {
		return nil, err
	} else if item == nil {
		return nil, errors.ErrNotFound
...
func (a *UserSrv) checkName(ctx context.Context, item schema.User) error {
	if item.Name == schema.GetRootUser().Name {
